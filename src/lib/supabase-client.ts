...
export async function loadUserCloudConfig(): Promise<UserCloudConfig | null> {
  const { data, error } = await supabase
    .from("user_configs")
    .select("supabase_url, supabase_api_key, token_exact")
    .maybeSingle();

  if (error || !data) return null;

  const config: UserCloudConfig = {
    url: data.supabase_url,
    apiKey: data.supabase_api_key,
    tokenExact: data.token_exact ?? null,
  };

  saveSupabaseConfig({ url: config.url, apiKey: config.apiKey });
  return config;
}
...
export async function saveExactTokenToCloud(tokenExact: string) {
  ...
  if (!currentConfig?.supabase_url || !currentConfig?.supabase_api_key) {
    throw new Error("Configure o Supabase antes de salvar o token do Exact Spotter.");
  }
  ...
  const { error } = await supabase.from("user_configs").upsert(
    {
      ...
      supabase_api_key: currentConfig.supabase_api_key,
      token_exact: tokenExact.trim(),
    },
    { onConflict: "user_id" }
  );
...
export async function getExactTokenFromCloud(): Promise<string | null> {
  const { data, error } = await supabase
    .from("user_configs")
    .select("token_exact")
    .maybeSingle();

  if (error || !data?.token_exact) return null;
